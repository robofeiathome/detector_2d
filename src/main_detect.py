            ...
            if time_elapsed > 1. / frame_rate:
                ...
                if self._current_image is not None:
                    try:
                        ...
                        objects.sort(key=lambda x: x.mask.sum() if x.mask is not None else 0, reverse=True)
                        ...
                        for obj in objects:
                            ...
                            if obj.mask is not None:
                                coords = np.where(obj.mask)
                                aux.image_x.data = int(np.mean(coords[1]))
                                aux.image_y.data = int(np.mean(coords[0]))
                                aux.image_width.data = int(coords[1].max() - coords[1].min())
                                aux.image_height.data = int(coords[0].max() - coords[0].min())
